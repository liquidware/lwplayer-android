...
static int attribute_align_arg avcodec_decode_audio3_audio_thread(AVCodecContext *avctx, int16_t *samples,
                         int *frame_size_ptr,
                         AVPacket *avpkt)
{
    int ret;
    ...
    if((avctx->codec->capabilities & CODEC_CAP_DELAY) || avpkt->size){
        ...
        if(*frame_size_ptr < AVCODEC_MAX_AUDIO_FRAME_SIZE){
        	LOGE("A:Error buffer smaller than AVCODEC_MAX_AUDIO_FRAME_SIZE\n");
            return -1;
        }
        if(*frame_size_ptr < FF_MIN_BUFFER_SIZE ||
        *frame_size_ptr < avctx->channels * avctx->frame_size * sizeof(int16_t)){
            LOGE("A:Error buffer %d too small\n", *frame_size_ptr);
            return -1;
        }
...
int audio_decode_frame() {
    ...
    while (avpkt.size > 0) {
        ...
        len = avcodec_decode_audio3_audio_thread(c, (short *)(outbuf+outbuf_size), &out_size, &avpkt);
        if (len < 0) {
