...
import com.liquidware.lwplayer.MediaThread;
...
//    private ProgressBar progress;
    ...
    private static String TAG = "AACPlayerActivity";
...
/*
    ...
    private void start( int decoder ) {
        ...
        while (1==1) {
         	try { 
	        	...
	        	if (mediaThread.getAsfInputStream() != null) {
	        		try {
	        			bytesAvailable = mediaThread.getAsfInputStream().available();
						Log.d(TAG, "getAsfInputStream available=" + bytesAvailable);
						if (bytesAvailable > 1500) {
							break;
						}
					} catch (IOException e) {
	        		...
	        		Log.e(TAG, "getAsfInputStream available=null");
	        	}
        	} catch(Exception e) { 
        		e.printStackTrace();
        	}
    	...
    	aacPlayer.playAsync( mediaThread.getAsfInputStream());
    ...
    private void stop() {
    	try {
    		Log.d(TAG, "Stopping...");
    		if (mediaThread !=  null) { mediaThread.stop(); }
			try { 
				Thread.sleep(5000); Thread.yield(); 
			} catch(Exception ex) { }	
	        if (aacFileChunkPlayer != null) { aacFileChunkPlayer.stop(); aacFileChunkPlayer = null; }
	        if (aacPlayer != null) { aacPlayer.stop(); aacPlayer = null; }
